-- Atomic browser bookmark import. The raw HTML never reaches Postgres; only
-- the validated DTO produced by the browser is accepted.
...
begin
  if auth.uid() is null or jsonb_typeof(p_tags) <> 'array' or jsonb_typeof(p_items) <> 'array'
     or jsonb_array_length(p_tags) > 500 or jsonb_array_length(p_items) > 500 then
