...
/**
 * Claims up to `limit` (capped at 6) pending jobs and processes them
 * 3-at-a-time. Safe to call with nothing pending (returns all zeros). Pass
 * `itemIds` to scope the claim to a set of items (e.g. the current page);
 * omit it to keep the old global sweep.
 *
 * Not a Server Action (no `"use server"` here or anywhere in this module):
 * it is called both from other Server Actions running server-side
 * (`lib/actions/items.ts`'s post-create `after()` sweep) and from
 * `app/api/previews/drain/route.ts`, which is the only path the client
 * hits -- see that route's header comment for why the client goes through
 * a plain Route Handler instead of invoking this as a Server Action
 * directly.
 */
